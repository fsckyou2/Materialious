...
export async function GET(event) {
	...
	const height = Number.isFinite(maxHeight) ? maxHeight : 1080;
	const baseUrl = castBaseUrl(event, session.id);

	// Live has no index to build a byte ranged manifest from, so it gets a
	// numbered, dynamic one instead.
	const manifest =
		session.source === 'live'
			? await session.getLiveManifest(baseUrl, profile, height)
			: await session.getManifest(
					baseUrl,
					profile,
					height,
